...
CHRONIC.showAlert = function (title, msg, c)
{
...
CHRONIC.doCheckInsert = function()
{
	...
	if( _check ) 
	{
		CHRONIC.showAlert(' เกิดข้อผิดพลาด ',  ' กรุณาตรวจสอบข้อมูลเหล่านี้  '  + _str_error  , 'alert alert-error');
	} 
	else 
	{
		CHRONIC.doSave( _vn, _weight, _height, _waist, _sbp, _dbp, _foot, _eye );
	}	
};

CHRONIC.doSave = function( _vn, _weight, _height, _waist, _sbp, _dbp, _foot, _eye	) 
{
	// post data with ajax
	$.ajax({
		...
		success: function(data)
		{
			if(data.success)
			{
				CHRONIC.showAlert(' บันทึกข้อมูล',  ' บันทึกข้อมูลเสร็จเรียบร้อยแล้ว', 'alert alert-success');
				// reset form
			...
			{
				CHRONIC.showAlert('เกิดข้อผิดพลาด!',  data.status,  'alert alert-error');
			}
		},
		error: function(xhr, status, errorThrown){
			CHRONIC.showAlert('เซิร์ฟเวอร์มีปัญหา!', 'เกิดข้อผิดพลาดในการส่งข้อมูล  : <code>' +  xhr.status + ': ' + xhr.statusText + '</code>' ,'alert alert-error');
	  }	
	});
};

CHRONIC.setChronicVisitScreen = function() 
{
...
CHRONIC.doRemove = function( _id ) 
{
	$.ajax({
		...
		success: function(data){
			if(data.success)
			{
				CHRONIC.showAlert(' ผลการลบ',  ' ลบรายการที่ไม่ต้องการเรียบร้อยแล้ว ', 'alert alert-success');
				CHRONIC.getList();
			}
			else
			{
				CHRONIC.showAlert('เกิดข้อผิดพลาด!', 'เกิดข้อผิดพลาดในการส่งข้อมูล:  ' + data.status,  'alert alert-error');
			}
		  
		},
		error: function(xhr, status, errorThrown)
		{
			CHRONIC.getList('เซิร์ฟเวอร์มีปัญหา!', 'เกิดข้อผิดพลาดในการส่งข้อมูล  : <code>' +  xhr.status + ': ' + xhr.statusText + '</code>' ,'alert alert-error');
	  }	
	});
};
...
$(function(){
	...
	$('a[data-name="service-chronicfu"]').click(function(){
		...
		CHRONIC.setChronicVisitScreen();
	});
	
	$( 'button[data-name="btn-save-chronicfu"]' ).click( function() {
		CHRONIC.doCheckInsert();
	});

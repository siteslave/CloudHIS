var DRUG = {};
DRUG.showAlert = function (title, msg, c)
{
...
DRUG.doCheckInsert = function()
{
	...
	{
		DRUG.doSave( _vn, _drug_id, _price, _qty, _usage_id, _drug_name, _usage_name );
	}	
};

DRUG.doSave = function( _vn, _drug_id, _price, _qty, _usage_id, _drug_name, _usage_name ) 
{
	$.ajax({
		...
		success: function(data){
			if(data.success){
				...
				DRUG.showAlert('เกิดข้อผิดพลาด!', 'เกิดข้อผิดพลาดในการส่งข้อมูล:  ' + data.status,  'alert alert-error');
			}
		},
		error: function(xhr, status, errorThrown){
			DRUG.showAlert('เซิร์ฟเวอร์มีปัญหา!', 'เกิดข้อผิดพลาดในการส่งข้อมูล  : <code>' +  xhr.status + ': ' + xhr.statusText + '</code>' ,'alert alert-error');
	  }	
	});// ajax
};

DRUG.doRemove = function( _vn, _drug_id) 
{
	$.ajax({

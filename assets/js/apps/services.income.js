...
INCOME.showAlert = function (title, msg, c){
...
INCOME.doCheckInsert = function()
{
	...
	if( ! _vn ) 
	{
		INCOME.showAlert('กรุณาตรวจสอบข้อมูล', 'ไม่พบ <code> รหัสการให้บริการ (VN) </code>',  'alert alert-error');
	} 
	else if( ! _income_id ) 
	{
		INCOME.showAlert('กรุณาตรวจสอบข้อมูล', 'ไม่พบ<code>รายการค่าใช้จ่าย</code>',  'alert alert-error');
	} 
	else if( ! _qty ) 
	{
		INCOME.showAlert('กรุณาตรวจสอบข้อมูล', 'ไม่พบ<code>จำนวน</code>',  'alert alert-error');
	} 
	else if( ! _price ) 
	{
		INCOME.showAlert('กรุณาตรวจสอบข้อมูล', 'ไม่พบ<code>ราคา</code>',  'alert alert-error');
	} 
	else 
	{
		INCOME.doSave( _vn, _income_id, _price, _qty, _income_name, _income_unit );
	}	
};

INCOME.doSave = function(_vn, _income_id, _price, _qty, _income_name, _income_unit ) {
	...
	$.ajax({
		...
		success: function(data)
		{
			if(data.success)
			{
				INCOME.showAlert(' บันทึกข้อมูล',  ' บันทึกข้อมูลเสร็จเรียบร้อยแล้ว', 'alert alert-success');
			...
			{
				INCOME.showAlert('เกิดข้อผิดพลาด!', 'กรุณาตรวจสอบข้อมูล:  ' + data.status,  'alert alert-error');
			}
		  
		},
		error: function(xhr, status, errorThrown){
			INCOME.showAlert('เซิร์ฟเวอร์มีปัญหา!', 'เกิดข้อผิดพลาดในการส่งข้อมูล  : <code>' +  xhr.status + ': ' + xhr.statusText + '</code>' ,'alert alert-error');
			//console.log(xhr);
	  }	
	});
};

INCOME.doRemove = function( _vn, _income_id) {
	$.ajax({
		...
		success: function(data){
			if(data.success){
				INCOME.showAlert(' ผลการลบ',  ' ลบรายการที่ไม่ต้องการเรียบร้อยแล้ว : หากข้อมูลไม่อัปเดทกรุณารีเฟรชหน้าเพจใหม่ ', 'alert alert-success');
			}else{
				INCOME.showAlert('เกิดข้อผิดพลาด!', 'เกิดข้อผิดพลาดในการส่งข้อมูล:  ' + data.status,  'alert alert-error');
			}

...
class People extends CI_Controller {
	...
	*********************************************************************/
	public function getdetail()
	{
	  ...
	  if( ! empty($id) )
	  {
	    $result = $this->People->_get_detail( $id );
	    
	    if ( $result ) $json = '{"success": true, "rows": ' . json_encode( $result ) . '}';
	    else $json = '{"success": false, "statusText": "Model error, please contract administrator."}';
  ...
  public function getlist()
  {
    ...
    if( ! empty($house_id) )
    {
      ...
      if ( $result ) $json = '{"success": true, "rows": ' . json_encode( $result ) . '}';
      else $json = '{"success": false, "statusText": "Model error or not found."}';
	...
	public function dosave()
	{
      ...
      // assign data to array
			...
			else // do add new
			{
			  ...
			  else $json = '{"success": false, "statusText": "ข้อมูลเลขบัตรประจำตัวประชาชน ซ้ำ"}';
			}
